...
export class FastChecker {
  ...
  private formatInboxMessage(msg: InboxMessage): string {
    ...
    // the header line, not a fence). The [delivered:] stamp salts the content
    // per delivery attempt so a 5-min stale REDELIVERY is not swallowed by
    // MessageDedup (redelivery contract — see doc comment above).
    const deliveredAt = new Date().toISOString();
    const safeFrom = sanitizeForPtyInjection(msg.from);
    return `=== AGENT MESSAGE from ${safeFrom}${replyNote} [msg_id: ${msg.id}] [delivered: ${deliveredAt}] ===
...
`;
  ...
  private async checkUrgentSignal(): Promise<void> {
    ...
    if (existsSync(urgentPath)) {
      try {
        ...
        // Fence the body unescapably (#592 follow-up) so a signal payload
        // carrying its own fence can't break out and forge daemon containment
        // headers. Delivery semantics keep the dispatch-bug fix (2026-06-07):
        // inject first, unlink only on success; NOT_RUNNING retains for retry.
  ...
  private async checkContextStatus(): Promise<void> {
    ...
    if (effectivePct >= handoff && this.ctxHandoffFiredAt === 0 && !withinHandoffGrace) {
      ...
      // Cooperative-restart loop backstop: count handoff fires in a persisted 15min window.
      // If they reach 3, the runtime is not resetting context on restart — trip the circuit
      // breaker (30min pause) instead of handing off again so the loop self-limits.
      // Cap 3 is above the benign 1-2 fires a single very-large turn can produce before settling.
