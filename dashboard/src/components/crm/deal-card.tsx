...
export function DealCard({ deal, onClick, onDelete }: DealCardProps) {
  return (
    <div
      ...
      className={cn(
        'w-full rounded-lg border bg-card p-3 text-left transition-colors hover:bg-accent/50 cursor-pointer',
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {onDelete && (
          <button
            onClick={(e) => { e.stopPropagation(); if (confirm(`Delete "${deal.title}"?`)) onDelete(deal.id); }}
