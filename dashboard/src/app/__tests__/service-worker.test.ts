...
describe("service worker", () => {
  ...
  it("handles build assets + icons (cache-first)", () => {
  ...
  it("activate only evicts its own cortextos-* old caches (not origin-wide)", async () => {
    sw.caches.keys.mockResolvedValueOnce([
      "cortextos-v0", // ours, old → delete
      "cortextos-v1", // ours, current → keep
      "some-other-app-cache", // not ours → keep (codex P3)
    ]);
    await runLifecycle(sw.listeners.activate);
    const deleted = sw.caches.delete.mock.calls.map((c) => c[0]);
    expect(deleted).toEqual(["cortextos-v0"]);
  });
